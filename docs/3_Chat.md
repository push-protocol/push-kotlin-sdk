...
### **Signer**
#### For the user's PGP secret key encryption and decryption Singer capable of doing eip-191 signature. The Signer protocol defines an abstract signer that can be implemented to provide signing functionality. This protocol provides two methods: getEip191Signature and getAddress
```kotlin
    ...
    val signature = PushUser.Signature.getEip191Signature(privateKey,message)
    val address = PushUser.Signature.getAddress(privateKey)
```
...
```kotlin
    ...
    val decryptedPgpKey = PushUser.DecryptPgp.decryptPgpKey(privateKey,encryptedPrivateKey )
```
